...
function DroneIcon({ size = 24 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m9.2 10.1 5.6 0 1.6 2.5H7.6l1.6-2.5Z" /><path d="m8.1 12.6-2 2.1m9.8-2.1 2 2.1M7.2 15.7l3.1-1.6m6.5 1.6-3.1-1.6M10 12.6l-1 4.3m5-4.3 1 4.3M10.1 16.9h3.8" /><circle cx="4.5" cy="16.2" r="1.5" /><circle cx="19.5" cy="16.2" r="1.5" /><path d="M5.6 14.6H3.3m17.4 0h-2.3" /></svg>;
}
...
export function LandingPage() {
  ...
  return <main>
    ...
    <section className="section fleet" id="aeronaves"><div className="shell"><motion.div className="section-heading section-heading--center section-heading--light" {...entrance(reducedMotion)}><span className="eyebrow">TECNOLOGIA PARA CADA OPERAÇÃO</span><h2>Quais aeronaves operamos</h2><p>Selecionamos o equipamento mais adequado de acordo com a área, o terreno e o objetivo da aplicação.</p></motion.div><div className="fleet__grid">{aircraft.map(([alias, model, title, text, indicator, image], index) => <motion.article {...entrance(reducedMotion, index * 0.08)} whileHover={cardHover(reducedMotion)} className="aircraft-card" key={model}><><div className="aircraft-card__image"><Image src={image} alt={`${model} usado em ${title.toLocaleLowerCase("pt-BR")}`} fill sizes="(max-width: 767px) 100vw, (max-width: 1100px) 50vw, 25vw" /><span>{alias}</span></div><h3>{model}</h3><h4>{title}</h4><p>{text}</p><b>{indicator}</b></></motion.article>)}</div><motion.aside {...entrance(reducedMotion, 0.16)} className="fleet-owner"><div className="fleet-owner__icon"><DroneIcon /></div><div><span className="eyebrow">OPERAÇÃO COM SUA AERONAVE</span><h3>Você tem drone e não tem piloto?</h3><p>Não tem problema. Nossa equipe pode operar o seu drone com planejamento, segurança e acompanhamento em campo.</p></div><WhatsAppCta className="fleet-owner__cta" label="FALAR SOBRE MINHA AERONAVE" compact /></motion.aside><p className="fleet__notice">A produtividade pode variar conforme as condições e os parâmetros da operação.</p></div></section>
